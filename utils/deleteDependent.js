...
let Authority = require('../model/authority');
let License = require('../model/license');
...
const deleteAuthority = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(Authority,filter);
    return response;
  } catch (error){
...
const deleteLicense = async (filter) =>{
  try {
    let response  = await dbService.deleteMany(License,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const authorityFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const authorityCnt = await dbService.deleteMany(Authority,authorityFilter);

      const licenseFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const licenseCnt = await dbService.deleteMany(License,licenseFilter);
...
const countAuthority = async (filter) =>{
  try {
    const authorityCnt =  await dbService.count(Authority,filter);
    return { authority : authorityCnt };
  } catch (error){
    throw new Error(error.message);
  }
};

const countLicense = async (filter) =>{
  try {
    const licenseCnt =  await dbService.count(License,filter);
    return { license : licenseCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const authorityFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const authorityCnt =  await dbService.count(Authority,authorityFilter);

      const licenseFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const licenseCnt =  await dbService.count(License,licenseFilter);
...
const softDeleteAuthority = async (filter,updateBody) =>{  
  try {
    const authorityCnt =  await dbService.updateMany(Authority,filter);
    return { authority : authorityCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteLicense = async (filter,updateBody) =>{  
  try {
    const licenseCnt =  await dbService.updateMany(License,filter);
    return { license : licenseCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const authorityFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const authorityCnt = await dbService.updateMany(Authority,authorityFilter,updateBody);

      const licenseFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const licenseCnt = await dbService.updateMany(License,licenseFilter,updateBody);
